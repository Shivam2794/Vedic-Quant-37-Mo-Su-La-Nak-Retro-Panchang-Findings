...
from eternal_quant_evolution_v5 import (
...
PROVEN_EDGE_FINDINGS: Dict[int, float] = {
    2:  0.0963,   # best IC (IWM H=21d)
    5:  0.1099,   # best IC (IWM H=21d) — STRONGEST anomaly
    6:  0.0590,   # best IC (IWM H=10d)
    9:  0.0507,   # best IC (SPY H=21d)
    16: 0.0719,   # best IC (IWM H=21d)
    19: 0.0521,   # best IC (IWM H=10d)
    22: 0.0580,   # best IC (QQQ H=21d)
    24: 0.0514,   # best IC (SPY H=21d)
    26: 0.0746,   # best IC (QQQ H=21d)
    27: 0.0625,   # best IC (SPY H=21d)
    35: 0.0521,   # best IC (IWM H=21d)
}

# 11 Confirmed NOISE findings — pre-silence at generation 0 (L1 frozen = 0)
NOISE_FINDINGS: List[int] = [3, 7, 12, 15, 17, 18, 28, 29, 31, 32, 34]

# 15 WATCH findings — standard random initialization
WATCH_FINDINGS: List[int] = [1, 4, 8, 10, 11, 13, 14, 20, 21, 23, 25, 30, 33, 36, 37]
...
def run_phase4_evolution(
    ...
    checkpoint_path: str = "eternal_best_model_v5_phase4.json",
) -> Tuple[V5Genome, List[V5Genome], List[Dict]]:
    """
    Phase 4: IC-Guided 50-Generation NSGA-II Evolution.
    ...
    pool = ctx.Pool(processes=n_workers, initializer=init_worker,
                    initargs=(X, opens, highs, lows, closes))
    ...
    for gen in range(1, generations + 1):
        ...
        for idx in sorted_idx[:num_lamarck]:
            evaluated[idx] = optimize_genome_lamarckian(
                evaluated[idx], X, opens, highs, lows, closes, maxiter=30
            )
    ...
    champion = max(population, key=lambda g: g.net_return)
    final_path = os.path.join(PROJECT_ROOT, checkpoint_path)
    save_checkpoint_atomic(champion, final_path)
    print(f"\n[Phase 4] Final champion saved: {final_path}")
    print(f"[Phase 4] Champion CAGR: {champion.cagr*100:.2f}% | MaxDD: {champion.max_dd*100:.2f}%")
...
def run_fold_evolution(
    ...
    pool = ctx.Pool(
        ...
        initargs=(X_is, opens_is, highs_is, lows_is, closes_is)
    )
...
def run_phase5_walkforward(
    ...
    while True:
        ...
        champion = run_fold_evolution(
            X_is, opens_is, highs_is, lows_is, closes_is,
        ...
        oos_cagr, oos_dd = evaluate_backtest(
            ...
            opens_oos, highs_oos, lows_oos, closes_oos,
            champion.stop_loss, champion.take_profit,
            champion.max_leverage, champion.v_th
        )
        oos_sharpe = compute_oos_sharpe(X_oos, champion, closes_oos)
...
def main() -> None:
    ...
    # -- Step 2: Phase 4 — IC-Guided Evolution (50 gens, pop 150)
    champion, final_pop, gen_log = run_phase4_evolution(
        X, opens, highs, lows, closes,
        feature_cols,
        pop_size=150,
        generations=50,
        checkpoint_path="eternal_best_model_v5_phase4.json",
        seed=42,
    )

    print(f"\n[Phase 4 Complete] Champion CAGR: {champion.cagr*100:.2f}% | MaxDD: {champion.max_dd*100:.2f}%")

    # -- Step 3: Phase 5 — Walk-Forward OOS Validation
    folds = run_phase5_walkforward(
        X, opens, highs, lows, closes, feature_cols
    )

    # -- Step 4: Write reports
    report_path = os.path.join(PROJECT_ROOT, "v5_walkforward_report.md")
    ...
    print("\nPhase 4 & 5 Complete. Check v5_walkforward_report.md for full results.")
