"""
MASTER GRINDER V11 — The Final Singularity
=============================================================================
BRUTAL INSPECTOR FIXES APPLIED (V11):

FIX #1: Trade Count DataFrame Overhaul. V10's `(is_active.diff() == 1)` failed to count 
        back-to-back sequential trades without a cash day in between. `strategy_*` functions 
        now return a `pd.DataFrame` containing both `pnl` and explicitly tracked `trades`, 
        ensuring 100% statistically accurate trade thresholds.
FIX #2: Regulatory Fee Denominator Bug. Previously, `(SEC_FEE + FINRA_TAF) / exit_price` 
        incorrectly divided the SEC percentage fee by the absolute dollar price, massively 
        undercharging SEC fees. The true formula `exit * (1 - SEC_PCT) - FINRA_TAF_SHARE` is now applied.
"""
import threading
...
# ============================================================
def compute_metrics(daily_df: pd.DataFrame, min_trades: int = MIN_TRADES) -> dict:
    """Compute all performance metrics. Uses actual calendar years for CAGR."""
    if isinstance(daily_df, pd.DataFrame) and 'trades' in daily_df:
        trades = int(daily_df['trades'].sum())
        daily_pnl = daily_df['pnl']
    else:
        daily_pnl = daily_df
        is_active = (daily_pnl != 0).astype(int)
        trades = int((is_active.diff() == 1).sum() + is_active.iloc[0]) if len(is_active) > 0 else 0
...
def walk_forward_check(daily_df: pd.DataFrame, n_windows: int = 3,
    ...
    for i in range(n_windows):
        window = daily_df.iloc[i * size: (i + 1) * size]
        m = compute_metrics(window, min_trades=min_trades_per_window)
        if m['Sharpe'] < min_sr:
            return False
    return True
...
def strategy_overnight(signal_df: pd.DataFrame, params: dict) -> pd.Series:
    ...
    return pd.DataFrame({'pnl': pnl, 'trades': mask.astype(int)}, index=df.index)
...
def strategy_grand_portfolio(signal_df: pd.DataFrame, params: dict) -> pd.Series:
    ...
    on_pnl = strategy_overnight(signal_df, {
...
def strategy_multi_day_trend(signal_df: pd.DataFrame, params: dict) -> pd.Series:
    ...
    return pd.DataFrame({'pnl': np.nan_to_num(pnl, nan=0.0), 'trades': trade_starts}, index=df.index)
...
def main():
    print("="*60)
    print("MASTER GRINDER V11 — MULTI-CORE ETERNAL HOLY GRAIL ENGINE")
    print("="*60)
    ...
    study = optuna.create_study(
        study_name="v11_singularity", direction='maximize',
        storage="sqlite:///grinder_v11.db", load_if_exists=True,
    ...
    study.optimize(
        ...
        callbacks=[make_save_callback('grinder_v11_eternal_best.json'), holy_grail_callback]
    )
    ...
    with open('grinder_v11_FINAL_VERDICT.json', 'w') as f:
        json.dump(final_json, f, indent=4)
    print("\n[SAVED] grinder_v11_FINAL_VERDICT.json written.")
