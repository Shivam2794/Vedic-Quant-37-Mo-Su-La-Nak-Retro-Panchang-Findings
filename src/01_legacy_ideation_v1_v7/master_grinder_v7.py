"""
MASTER GRINDER V6 — Institutional Margin & Size Leak Fix
=============================================================
BRUTAL INSPECTOR FIXES APPLIED (V6):

FIX #1: Multiday Size Leak. `actual_size` is now strictly locked on the entry day. 
        Previously, the dynamic `rolling_vol` caused the position size to subtly 
        rebalance every day without incurring slippage penalties.
FIX #2: Strict 100% Margin Cap in Grand Portfolio. Multiday and Intraday are now 
        forced to share the 1.0 (100%) max allocation pie. `md_max_size` is chosen 
        by the optimizer, and `id_max_size = 1.0 - md_max_size`. This mathematically 
        prevents TQQQ 600% exposure margin calls.
...
def main():
    ...
    study.optimize(
        ...
        callbacks=[make_save_callback('grinder_v6_eternal_best.json'), holy_grail_callback]
    )
    ...
    with open('grinder_v6_FINAL_VERDICT.json', 'w') as f:
        json.dump(final_json, f, indent=4)
    print("\n[SAVED] grinder_v6_FINAL_VERDICT.json written.")
