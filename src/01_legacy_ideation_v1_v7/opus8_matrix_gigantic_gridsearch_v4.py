    ...
    for i in numba.prange(N):
        ...
        for w in range(W):
            ...
            for t in range(start_is, end_is):
                ...
                t0 = 0.0
                # STRICT LOOKAHEAD PREVENTION: Use signals from t-1 to trade on day t
                ...
                # UNCONDITIONAL SCOPING: Update tracking vars regardless of position
                if t0 > 0: n_act_0 += 1
                prev_0 = t0
    ...
    for i in numba.prange(N):
        ...
        for t in range(start_is, end_is):
            ...
            for logic_idx in range(5):
                ...
                prevs[logic_idx] = t_val # Unconditional scoping!
...
def build_oos_return(mat_list, indices, logic_idx, rets, macro_overlay, vix_crash, apply_macro, start_oos, end_oos, initial_prev_inv):
    ...
    # Carry strategy position across boundaries to correctly charge turnover!
    ...
    for t in range(start_oos, end_oos):
        ...
        if t > 0 and not (apply_macro and vix_crash[t-1]):
            votes = sum([mat_list[k][t-1, indices[k]] for k in range(len(mat_list))])
            n_fams = len(mat_list)
...
def run_wfo():
    print("Starting Nested Walk-Forward Gridsearch (Absolution Edition v4)...")
    ...
    # No manual np.roll() needed here because we read from t-1 inside the loop!
    ...
    for ticker in UNIVERSE:
        ...
        # Track physical position across window boundaries
        current_position = 0.0
        
        # Precompute the single families to save O(W^2) waste
        precomputed_fams = {}
        ...
        for w in range(W):
            ...
            for fam in TREND_FAMILIES:
                ...
                # Filter out -999.0 valid combinations
            ...
            if best_pair_sharpe > 0:
                ...
                # No positive Sharpe, stay in cash
                # We must charge for exiting if current_position > 0!
