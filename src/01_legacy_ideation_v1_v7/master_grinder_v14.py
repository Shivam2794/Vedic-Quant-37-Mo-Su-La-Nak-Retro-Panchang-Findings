...
# ============================================================
def make_cpcv_objective(signal_df: pd.DataFrame, n_splits=5, purge=10, embargo=20):
    ...
    def objective(trial: optuna.Trial) -> float:
        params = {
            'rsi_min': trial.suggest_float('rsi_min', 30, 50),
            'rsi_max': trial.suggest_float('rsi_max', 55, 80),
            'gap_atr_max': trial.suggest_float('gap_atr_max', 0.1, 1.5),
            'mom_z_min': trial.suggest_float('mom_z_min', -1.0, 1.0),
            'tp_atr_mult': trial.suggest_float('tp_atr_mult', 0.5, 3.0),
...
def main():
    print("="*60)
    print("V14: COMBINATORIAL PURGED CROSS-VALIDATION (CPCV) ENGINE")
    print("="*60)
    ...
    study.optimize(
        make_cpcv_objective(signal_df, n_splits=5, purge=10, embargo=20),
        n_trials=2000, 
    ...
    print("\n" + "="*60)
    print("V14 OPTIMIZATION COMPLETE")
    print("="*60)
    ...
    # Save Results
    with open('v14_cpcv_results.json', 'w') as f:
        json.dump({
            'cpcv_score': b.value,
    ...
    print("\n[SAVED] v14_cpcv_results.json written.")
