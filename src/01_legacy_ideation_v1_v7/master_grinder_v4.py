"""
MASTER GRINDER V4 — The Grand Portfolio Engine
=============================================================
BRUTAL INSPECTOR FIXES APPLIED (V4):

FIX #1: Leverage Ceiling Raised. MAX_POSITION_PCT increased from 0.50 to 1.0.
         This unleashes the CAGR potential of the ultra-selective multiday setup.
FIX #2: Grand Portfolio Architecture. V3 proved Multiday is the superior 
         standalone strategy, but it failed the 25% CAGR target. V4 stacks 
         Multiday (trend) + Intraday (mean reversion) geometrically.
FIX #3: Emoji Unicode crash fixed. All emojis removed from console outputs.

PREVIOUS FIXES INTACT:
  - Vectorized multiday loop (no O(N) Python loops)
  - RSI floor/ceiling
  - ROC Acceleration gate
  - Actual calendar year CAGR
    ...
    def objective(trial: optuna.Trial) -> float:
        ...
        if arch == 'grand_portfolio':
            params = {
                ...
                'md_target_vol': trial.suggest_float('md_target_vol', 0.20, 0.50),  # capped to leave room
                ...
                'id_target_vol': trial.suggest_float('id_target_vol', 0.20, 0.50),
            }
...
def main():
    ...
    # ── OPTION 2: Grand Portfolio Sweep (50k trials, Random) ──
    print("\n" + "="*60)
    print("OPTION 2: Grand Portfolio Stack (50k trials)")
    print("="*60)
    study2 = optuna.create_study(
        study_name="grand_portfolio", direction='maximize',
        sampler=optuna.samplers.RandomSampler(seed=99)
    )
    study2.optimize(
        make_objective(signal_df, arch_filter='grand_portfolio'),
        n_trials=50000, n_jobs=1, show_progress_bar=True,
        callbacks=[make_save_callback('grinder_v4_option2_grand.json')]
    )
    b2 = study2.best_params
    pnl2 = strategy_grand_portfolio(signal_df, b2)
    ...
    all_results['Option2_GrandPortfolio'] = {'metrics': m2, 'params': b2, 'wf': wf2}
    ...
    study3.optimize(
        ...
        callbacks=[make_save_callback('grinder_v4_option3_extended.json')]
    )
    b3 = study3.best_params
    a3 = b3.get('arch', 'multiday')
    if a3 == 'overnight': pnl3 = strategy_overnight(signal_df, b3)
    elif a3 == 'intraday': pnl3 = strategy_intraday_bracket(signal_df, b3)
    elif a3 == 'grand_portfolio': pnl3 = strategy_grand_portfolio(signal_df, b3)
    else: pnl3 = strategy_multi_day_trend(signal_df, b3)
    ...
    with open('grinder_v4_FINAL_VERDICT.json', 'w') as f:
        json.dump(final_json, f, indent=4)
    print("\n[SAVED] grinder_v4_FINAL_VERDICT.json written.")

    any_goal = any(v['GoalMet'] for v in final_json.values())
    print(f"\n[GOAL] CAGR>=25%, SR>=1.8, MDD<=-15%: {'ACHIEVED' if any_goal else 'Not yet'}")
    return final_json
