"""
MASTER GRINDER V10 — Ultimate Multi-Core Persistent Engine
=============================================================================
BRUTAL INSPECTOR FIXES APPLIED (V10):

FIX #1: Trade Count Overfitting Bug. `(daily_pnl != 0).sum()` counted ACTIVE DAYS, not TRADES. 
        A 10-day hold was counted as 10 trades, allowing strategies to bypass statistical significance limits. 
        Fixed by counting positive state transitions `(is_active.diff() == 1)`.
FIX #2: Missing Acceleration Parameter. `strategy_overnight` checked `ROC_Accel > 0`, but the optimizer 
        failed to pass `on_accel_min`. Now explicitly parameterized.
FIX #3: Multi-Core Deployment. Optuna `n_jobs=-1` deployed to blast through the 1M trial limit.
FIX #4: Immortal Optuna State. `storage='sqlite:///grinder_v10.db'` enabled to persist TPE probability trees across restarts!
"""
import threading
...
# ============================================================
def compute_metrics(daily_pnl: pd.Series, min_trades: int = MIN_TRADES) -> dict:
...
def main():
    print("="*60)
    print("MASTER GRINDER V10 — MULTI-CORE ETERNAL HOLY GRAIL ENGINE")
    print("="*60)
    ...
    study = optuna.create_study(
        study_name="v10_singularity", direction='maximize',
        storage="sqlite:///grinder_v10.db", load_if_exists=True,
    ...
    study.optimize(
        ...
        callbacks=[make_save_callback('grinder_v10_eternal_best.json'), holy_grail_callback]
    )
    ...
    with open('grinder_v10_FINAL_VERDICT.json', 'w') as f:
        json.dump(final_json, f, indent=4)
    print("\n[SAVED] grinder_v10_FINAL_VERDICT.json written.")
