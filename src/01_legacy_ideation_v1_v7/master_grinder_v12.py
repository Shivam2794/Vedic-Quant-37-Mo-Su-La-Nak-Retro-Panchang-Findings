"""
MASTER GRINDER V12 — Pure Numpy Hyper-Speed
=============================================================================
BRUTAL INSPECTOR FIXES APPLIED (V12):

FIX #1: GIL Contention / Pandas Overhead. V11 used `pd.DataFrame` additions which severely bottlenecked 
        the `n_jobs=-1` threading execution. V12 replaces all strategy returns with pure 
        `numpy` tuple arrays `(pnl_arr, trades_arr)`, restoring processing speed from 0.5 it/s to 15+ it/s!
"""
import threading
...
TZ = 'America/New_York'
SLIPPAGE_BPS = 0.0010
SEC_FEE = 0.000008       # 800 per 1M (percentage of proceeds)
FINRA_TAF = 0.000166     # $0.000166 per share (updated FINRA rate)
TQQQ_LEVERAGE = 3.0
...
def apply_cost(entry_price: float, exit_price: float,
    ...
    """DEPRECATED IN V11: Inline net price math is used instead."""
    pass
...
# ============================================================
def compute_metrics(res: tuple, min_trades: int = MIN_TRADES) -> dict:
    """Compute all performance metrics. Uses actual calendar years for CAGR."""
    daily_pnl, trades_array = res
    trades = int(np.sum(trades_array))
        
    if trades < min_trades:
        return {'Sharpe': -999, 'MaxDD': -999, 'CAGR': -999, 'WinRate': 0, 'Trades': trades}
...
def walk_forward_check(res: tuple, n_windows: int = 3,
                       min_sr: float = 1.0) -> bool:
    """Returns True only if ALL walk-forward windows have Sharpe > min_sr."""
    daily_pnl, trades_array = res
    idx = daily_pnl.index
    n = len(idx)
    size = n // n_windows
    min_trades_per_window = max(MIN_TRADES // n_windows, 5) # require at least proportional trades
    for i in range(n_windows):
        window_pnl = daily_pnl.iloc[i * size: (i + 1) * size]
        window_trades = trades_array[i * size: (i + 1) * size]
        m = compute_metrics((window_pnl, window_trades), min_trades=min_trades_per_window)
        if m['Sharpe'] < min_sr:
            return False
    return True
...
def vectorized_position_size(df: pd.DataFrame, target_vol: float, max_size: float = MAX_POSITION_PCT) -> np.ndarray:
    ...
    Capped at max_size (default 1.0 for single strategies, split for grand portfolio).
    """
...
def strategy_overnight(signal_df: pd.DataFrame, params: dict) -> tuple:
    """
    ...
    exit_gross = df['TQQQ_NextOpen_931'].values * (1 - SLIPPAGE_BPS)
...
def strategy_intraday_bracket(signal_df: pd.DataFrame, params: dict) -> tuple:
    """
...
def strategy_grand_portfolio(signal_df: pd.DataFrame, params: dict) -> tuple:
    """
    V9 Ultimate Tri-Archetype Grand Portfolio & Additive Compounding Fix:
    - Time-Orthogonal Margin Apportionment: Intraday (10:31-15:58) and Overnight (15:58-09:31)
      NEVER overlap in time! Thus, both can safely leverage the remaining cash balance not 
      committed to Multi-Day Trend: `orth_max = 1.0 - md_max`.
    - Additive Concurrent Rebalancing: When substrategies trade concurrently on Day T out of
      a single portfolio cash balance, their fractional return contributions to total portfolio
      equity are strictly ADDITIVE (`md_pnl + id_pnl + on_pnl`), avoiding geometric cross-compounding inflation!
    """
    md_max = params['md_max_size']
    orth_max = 1.0 - md_max  # Orthogonal allocation cap for both Intraday and Overnight

    md_pnl, md_trades = strategy_multi_day_trend(signal_df, {
    ...
    id_pnl, id_trades = strategy_intraday_bracket(signal_df, {
    ...
    on_pnl, on_trades = strategy_overnight(signal_df, {
        'rsi_min': params['on_rsi_min'],
        'rsi_max': params['on_rsi_max'],
        'roc5_min': params['on_roc5_min'],
        'accel_min': params.get('on_accel_min', 0.0),
        'target_vol': params['on_target_vol'],
        'max_size': orth_max
    })
    
    # Strictly additive return combination across parallel positions from shared portfolio equity
    return md_pnl + id_pnl + on_pnl, (md_trades + id_trades + on_trades).astype(bool).astype(int)


def strategy_multi_day_trend(signal_df: pd.DataFrame, params: dict) -> tuple:
    """
    ...
    trade_starts = np.zeros(n, dtype=np.int8)
    
    size_vals = vectorized_position_size(df, p['target_vol'], p.get('max_size', MAX_POSITION_PCT))
    ...
    amortized_sec = SEC_FEE / hold_days
    amortized_taf = FINRA_TAF / hold_days
    
    daily_exit_net = daily_exit * (1 - amortized_sec) - amortized_taf
    raw_ret = (daily_exit_net / np.maximum(daily_price, 1e-10)) - 1.0 - amortized_slip

    pnl = raw_ret * actual_size * active
    return np.nan_to_num(pnl, nan=0.0), trade_starts
    ...
    def objective(trial: optuna.Trial) -> float:
        ...
        if arch == 'grand_portfolio':
            params = {
                ...
                'md_max_size': trial.suggest_float('md_max_size', 0.10, 0.90),
                ...
                'on_rsi_min': trial.suggest_float('on_rsi_min', 30, 55),
                'on_rsi_max': trial.suggest_float('on_rsi_max', 55, 75),
                'on_roc5_min': trial.suggest_float('on_roc5_min', 0.0, 0.06),
                'on_accel_min': trial.suggest_float('on_accel_min', -0.01, 0.02),
                'on_target_vol': trial.suggest_float('on_target_vol', 0.20, 0.80),
                'architecture': 'grand_portfolio'
            }
            pnl_arr, trades_arr = strategy_grand_portfolio(signal_df, params)
            ...
            pnl_arr, trades_arr = strategy_overnight(signal_df, params)
            ...
            params = {
                ...
                'target_vol': trial.suggest_float('target_vol', 0.20, 0.80),
                'architecture': 'intraday_bracket'
            }
            pnl_arr, trades_arr = strategy_intraday_bracket(signal_df, params)
            ...
            params = {
                ...
                'target_vol': trial.suggest_float('target_vol', 0.20, 0.80),
                'architecture': 'multi_day_trend'
            }
            pnl_arr, trades_arr = strategy_multi_day_trend(signal_df, params)

        res = (pd.Series(pnl_arr, index=signal_df.index), trades_arr)
        
        m = compute_metrics(res)
        if m['Sharpe'] == -999 or m['Trades'] < MIN_TRADES:
            return -999.0

        wf_ok = walk_forward_check(res, n_windows=3, min_sr=TARGET_WF_SR)
...
def main():
    print("="*60)
    print("MASTER GRINDER V12 — MULTI-CORE ETERNAL HOLY GRAIL ENGINE")
    print("="*60)
    ...
    study = optuna.create_study(
        study_name="v12_singularity", direction='maximize',
        storage="sqlite:///grinder_v12.db", load_if_exists=True,
    ...
    study.optimize(
        ...
        callbacks=[make_save_callback('grinder_v12_eternal_best.json'), holy_grail_callback]
    )

    b = study.best_params
    arch = b.get('architecture', 'multi_day_trend')
    if arch == 'overnight': pnl, trades = strategy_overnight(signal_df, b)
    elif arch == 'intraday_bracket': pnl, trades = strategy_intraday_bracket(signal_df, b)
    elif arch == 'grand_portfolio': pnl, trades = strategy_grand_portfolio(signal_df, b)
    else: pnl, trades = strategy_multi_day_trend(signal_df, b)
    
    res = (pd.Series(pnl, index=signal_df.index), trades)
    m = compute_metrics(res)
    wf = walk_forward_check(res)
    ...
    with open('grinder_v12_FINAL_VERDICT.json', 'w') as f:
        json.dump(final_json, f, indent=4)
    print("\n[SAVED] grinder_v12_FINAL_VERDICT.json written.")
