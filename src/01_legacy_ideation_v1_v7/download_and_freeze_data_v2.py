...
def download_and_freeze():
    ...
    for ticker in TICKERS_TRADED:
        ...
        # STRICTLY FFILL INTERIOR HOLES ONLY (max 3 days for weekends) to prevent phantom trailing bars
        for col in ['Adj Close', 'Close']:
            ff = df_ticker[col].ffill(limit=3)
            bf = df_ticker[col].bfill()
            interior_mask = ff.notna() & bf.notna()
            df_ticker.loc[interior_mask, col] = ff[interior_mask]
