...
def objective(trial, df):
    ...
    if win_rate < 0.35: # Lowered to 35% since we expect XGBoost to bump it
        return -1.0 
...
def run_grinder(ticker):
    print(f"[{ticker}] Running V19 Master Grinder (Fixed Same-Day Exit & Gap Slippage)...")
    latent_file = f"{ticker}_daily_V19_latent.parquet"
    ...
    out_file = f"{ticker}_V19_trades.parquet"
