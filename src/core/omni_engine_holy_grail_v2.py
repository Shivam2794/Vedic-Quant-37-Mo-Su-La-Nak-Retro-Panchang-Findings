...
def run_holy_grail_v2():
    print("[*] Downloading Leveraged All-Weather Data...")
    ...
    # Base portfolio: 30% UPRO, 55% TMF, 15% GLD
    base_ret = returns['UPRO'] * 0.30 + returns['TMF'] * 0.55 + returns['GLD'] * 0.15
    
    # 20-day annualized volatility of the base portfolio
    vol20 = base_ret.rolling(20).std() * np.sqrt(252)
    
    TARGET_VOL = 0.10 # Reduced to 10% to meet <20% DD constraint
    ...
    for date in month_ends:
        ...
        weights.loc[date, 'UPRO'] = 0.30 * multiplier
        weights.loc[date, 'TMF'] = 0.55 * multiplier
        weights.loc[date, 'GLD'] = 0.15 * multiplier
    ...
    print("========================================================")
    print(f"[*] LEVERAGED ALL-WEATHER VOL-TARGETED RESULTS")
    print(f"CAGR: {cagr:.2%}")
