...
def check_astrology_folders():
    ...
    try:
        ...
        folders_to_check = ['horoscope-chart/', 'kp/', 'vimshottari-dasha/', 'consolidated/']

        for folder in folders_to_check:
            print(f"\n--- Checking inside '{folder}' ---")
            response = s3.list_objects_v2(Bucket=bucket_name, Prefix=folder, MaxKeys=5)
