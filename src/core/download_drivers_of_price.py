"""
Download "The drivers of price and answers to them" (course ID: e5f9d701)
from Quant Rick's Skool classroom.
"""
import os
import json
...
def plain_desc(raw):
    """Extract plain text from Skool's v2 rich-text JSON desc field."""
    try:
...
# ── fetch course skeleton ─────────────────────────────────────────────────────
print(f'Fetching course: {BASE_URL}', flush=True)
data = fetch(BASE_URL)
if not data:
    print('ERROR: could not load course page.', flush=True)
    raise SystemExit(1)
...
print(f'Output dir   : {course_dir}', flush=True)
print('=' * 60, flush=True)
...
for idx, child in enumerate(children):
    ...
    # fetch signed module page for playback token
    ...
    mod_rd = mod_data['props']['pageProps']['renderData']
    ...
    # ── save description as plain .txt ───────────────────────────────────────
    desc_raw = mod_rd.get('course', {}).get('course', {}).get('metadata', {}).get('desc', '')
    if not desc_raw:
        # fallback to looking recursively
        v2_list = []
        def find_v2(node):
...
print('\n' + '=' * 60, flush=True)
print(f'FINISHED: {ok_count} downloaded | {skip_count} skipped | {noVid_count} text-only | {fail_count} failed', flush=True)
print(f'Files in: {course_dir}', flush=True)
print('=' * 60, flush=True)
