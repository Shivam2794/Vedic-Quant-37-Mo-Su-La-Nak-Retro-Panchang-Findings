...
import json
from current_fable_strategy import get_optuna_params, strategy_logic
...
with open('FINAL_WINNING_STRATEGY.py', 'w') as f:
    with open('current_fable_strategy.py', 'r') as r:
        f.write(r.read())
...
plt.plot(pd.to_datetime(cum.index), cum.values, color='cyan')
plt.title(f"Coiled Spring Alpha (10 BPS Slip)\nSharpe {m['Sharpe']:.2f} | DD {m['MaxDD']*100:.1f}%")
plt.grid(True, alpha=0.3)
plt.yscale('log')
plt.savefig('../brain/662b40cd-41f4-48b4-8be0-781b58f55b00/coiled_spring_equity.png')
