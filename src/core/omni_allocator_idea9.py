"""
OMNI-ALLOCATOR IDEA 9: PATIENCE DIAL
Implements the Tri-Asset (60/20/20) institutional core with Fable Idea 9:
The Patience Dial applies an Exponential Moving Average (EMA) smoother with tau=0.85
to the target allocation weights to eliminate high-frequency whip-sawing.
...
def run_idea9():
    ...
    # IDEA 9: PATIENCE DIAL (tau=0.85 EMA smoothing on weights)
    w_idea9_smooth = w_df.ewm(alpha=0.15, adjust=False).mean()
    
    w_target = w_idea9_smooth.loc[valid_idx].fillna(0.0).values
    ...
    print("\n" + "="*80)
    print("IDEA 9: PATIENCE DIAL PERFORMANCE (MODE B 1-BAR LAG)")
    print("="*80)
