...
def run_plot():
    ...
    def eval_weights(w_df, use_cppi=False, floor_pct=0.86):
        ...
        strat_s = pd.Series(port_ret, index=valid_idx)
        spy_s = pd.Series(r_mat[:, 2], index=valid_idx)
        
        strat_m = strat_s.resample('ME').apply(lambda x: np.prod(1+x)-1)
        spy_m = spy_s.resample('ME').apply(lambda x: np.prod(1+x)-1)
        beat_rate = (strat_m > spy_m).mean()
        
        return cagr, sharpe, dd, beat_rate, pd.Series(cum, index=valid_idx)

    # Calculate SPY benchmark cumulative
    spy_ret = pd.Series(r_mat[:, 2], index=valid_idx)
    spy_cum = np.cumprod(1 + spy_ret)
    spy_cagr = np.prod(1 + spy_ret) ** (252 / len(valid_idx)) - 1
    spy_cummax = np.maximum.accumulate(spy_cum)
    spy_dd = np.min((spy_cum - spy_cummax) / spy_cummax)
    spy_excess = spy_ret.values - (cy_arr * delta_days)
    spy_sr = np.sqrt(252) * np.mean(spy_excess) / np.std(spy_ret.values, ddof=1)
    
    # 1. Idea 4: CPPI
    c4, s4, d4, b4, cum4 = eval_weights(w_base, use_cppi=True, floor_pct=0.86)
    
    # 2. Idea 9: Patience Dial
    w_idea9_smooth = w_base.ewm(alpha=0.15, adjust=False).mean()
    c9, s9, d9, b9, cum9 = eval_weights(w_idea9_smooth)
    ...
    c5, s5, d5, b5, cum5 = eval_weights(w_idea5)

    print(f"SPY Benchmark -> CAGR: {spy_cagr*100:.2f}%, MaxDD: {spy_dd*100:.2f}%, Sharpe: {spy_sr:.2f}")
    print(f"Idea 4 (CPPI) -> CAGR: {c4*100:.2f}%, MaxDD: {d4*100:.2f}%, Sharpe: {s4:.2f}, SPY Beat Rate: {b4*100:.1f}%")
    print(f"Idea 9 (Patience Dial) -> CAGR: {c9*100:.2f}%, MaxDD: {d9*100:.2f}%, Sharpe: {s9:.2f}, SPY Beat Rate: {b9*100:.1f}%")
    print(f"Idea 5 (VoV Brake) -> CAGR: {c5*100:.2f}%, MaxDD: {d5*100:.2f}%, Sharpe: {s5:.2f}, SPY Beat Rate: {b5*100:.1f}%")
    
    plt.figure(figsize=(12, 7))
    plt.plot(cum4, label='Idea 4: Drawdown Governor (CPPI)', color='blue')
    plt.plot(cum9, label='Idea 9: Patience Dial (EMA)', color='orange')
    plt.plot(cum5, label='Idea 5: Vol-of-Vol Brake', color='green')
    plt.plot(spy_cum, label='SPY Benchmark', color='red', alpha=0.5, linestyle='--')
    plt.yscale('log')
    plt.title('Institutional Tri-Asset Strategies vs SPY Benchmark (2014-2024)')
    plt.ylabel('Cumulative Return (Log Scale)')
    plt.legend()
    plt.grid(True, which='both', alpha=0.3)
    plt.savefig('C:/Users/Shivam Patel/.gemini/antigravity/brain/d54a4837-63bb-49d2-bb84-fac829bdb2ba/equity_curves.png')
    plt.close()
