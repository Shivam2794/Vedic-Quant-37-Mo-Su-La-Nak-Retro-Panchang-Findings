...
def run_omni_allocator_v5():
    ...
    master_weights = pd.DataFrame(0.0, index=biz_idx, columns=['SPY', 'QQQ', 'TLT', 'GLD', 'BTC-USD'])
    ...
    for t in ['SPY', 'QQQ', 'TLT', 'GLD', 'BTC-USD']:
        ...
        master_weights[t] = trend * vol_w * 0.20 # Equal base weight allocation
        
    valid_idx = biz_idx[250:]
    master_weights = master_weights.loc[valid_idx]
    
    # ---------------------------------------------------------
    # 2. Execution Illusion Fix (Open-to-Open Returns)
    # ---------------------------------------------------------
    r_open = (open_prices / open_prices.shift(1)) - 1
    r_open = r_open.loc[valid_idx]
    
    # ---------------------------------------------------------
    # 3. Weight Drift Illusion Fix (Actual Delta Slippage)
    # ---------------------------------------------------------
    ...
    a_ret = (master_weights * r_open[['SPY', 'QQQ', 'TLT', 'GLD', 'BTC-USD']]).sum(axis=1)
    ...
    print("========================================================")
    print(f"[*] DYNAMIC OMNI-ALLOCATOR V5 (Ultimate Physics Edition)")
    print(f"CAGR: {cagr:.2%}")
