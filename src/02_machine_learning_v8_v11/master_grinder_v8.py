"""
MASTER GRINDER V8 — Institutional Margin & Trade Count Fix
=============================================================
BRUTAL INSPECTOR FIXES APPLIED (V8):

FIX #1: Trade Count Leak. `compute_metrics` previously checked `len(daily_pnl) < MIN_TRADES`, 
        which checks the number of *days* in the backtest, not the number of *trades*. 
        This allowed walk-forward windows to pass with as few as 1 or 2 trades. 
        It now strictly counts `(daily_pnl != 0).sum()` and enforces `min_trades` dynamically.
FIX #2: Multiday Size Leak. `actual_size` is now strictly locked on the entry day. 
FIX #3: Strict 100% Margin Cap in Grand Portfolio. Multiday and Intraday are now 
        forced to share the 1.0 (100%) max allocation pie. 
"""
...
def strategy_grand_portfolio(signal_df: pd.DataFrame, params: dict) -> pd.Series:
    """
    V6 Grand Portfolio: Strict Margin Apportionment.
    Optimizer chooses md_max_size (e.g. 0.60). id_max_size gets the rest (0.40).
    Sum of max exposure is guaranteed <= 1.0 (100% cash allocation).
    """
...
def main():
    print("="*60)
    print("MASTER GRINDER V8 — ETERNAL HOLY GRAIL ENGINE")
    print("="*60)
    ...
    study.optimize(
        ...
        callbacks=[make_save_callback('grinder_v8_eternal_best.json'), holy_grail_callback]
    )
    ...
    with open('grinder_v8_FINAL_VERDICT.json', 'w') as f:
        json.dump(final_json, f, indent=4)
    print("\n[SAVED] grinder_v8_FINAL_VERDICT.json written.")
