"""
MASTER GRINDER V9 — Ultimate Tri-Archetype & Orthogonal Margin Engine
=============================================================================
BRUTAL INSPECTOR FIXES APPLIED (V9):

FIX #1: Geometric Cross-Compounding Inflation Eliminated. Concurrently held daily allocations 
        from a single portfolio cash equity now combine strictly ADDITIVELY (`md_pnl + id_pnl + on_pnl`), 
        removing imaginary same-day cross-reinvestment returns.
FIX #2: Silent Margin Overflow Sealed. `strategy_overnight` and `strategy_intraday_bracket` previously 
        ignored `max_size` passed by the Grand Portfolio, silently defaulting to 100%. They now strictly 
        respect `p.get('max_size', MAX_POSITION_PCT)`.
FIX #3: Time-Orthogonal Tri-Archetype Integration. Overnight Momentum (15:58-09:31) never overlaps with 
        Intraday Bracket (10:31-15:58), allowing both to independently deploy `1.0 - md_max` without zero leverage overflow!
"""
...
def main():
    print("="*60)
    print("MASTER GRINDER V9 — ETERNAL HOLY GRAIL ENGINE")
    print("="*60)
    ...
    study.optimize(
        ...
        callbacks=[make_save_callback('grinder_v9_eternal_best.json'), holy_grail_callback]
    )
    ...
    with open('grinder_v9_FINAL_VERDICT.json', 'w') as f:
        json.dump(final_json, f, indent=4)
    print("\n[SAVED] grinder_v9_FINAL_VERDICT.json written.")
