...
def compile_v23_portfolio():
    print("Compiling v23 Concurrency-Aware Portfolio (with XGBoost Meta-Label Filtering)...")
    
    tickers = ['SPY']
    all_trades = []
    
    for ticker in tickers:
        trades_file = f"{ticker}_V23_trades.parquet"
        try:
    ...
    print(f"v23 Portfolio Win Rate: {win_rate:.2%}")
    print(f"v23 Portfolio Sharpe:   {sharpe:.2f}")
    
    df_port.to_parquet('portfolio_v23_executed_trades.parquet')
    print("Executed trades saved to portfolio_v23_executed_trades.parquet")

if __name__ == '__main__':
    compile_v23_portfolio()
