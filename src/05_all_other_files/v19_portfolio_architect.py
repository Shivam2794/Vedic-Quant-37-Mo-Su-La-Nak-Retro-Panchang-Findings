...
def compile_v19_portfolio():
    print("Compiling V19 Concurrency-Aware Portfolio (with XGBoost Meta-Label Filtering)...")
    
    tickers = ['SPY']
    all_trades = []
    
    for ticker in tickers:
        trades_file = f"{ticker}_V19_trades.parquet"
        try:
    ...
    print(f"V19 Portfolio Win Rate: {win_rate:.2%}")
    print(f"V19 Portfolio Sharpe:   {sharpe:.2f}")
    
    df_port.to_parquet('portfolio_v19_executed_trades.parquet')
    print("Executed trades saved to portfolio_v19_executed_trades.parquet")

if __name__ == '__main__':
    compile_v19_portfolio()
