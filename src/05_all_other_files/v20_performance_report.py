...
def generate_report():
    print("Generating v20 Mark-to-Market Performance Report...")
    trades_file = "portfolio_v20_executed_trades.parquet"
    ...
    df_latent = pd.read_parquet("SPY_daily_v20_latent.parquet") # To get exact dates
    
    # In v20, the trades are already filtered by the Meta-Labeler.
    ...
    plt.plot(strat_equity.index, strat_equity.values, label='v20 Strategy (OOS)', color='blue')
    plt.plot(benchmark.index, benchmark['Equity'].values, label='SPY Benchmark', color='gray', alpha=0.7)
    plt.title(f'v20 Strategy vs SPY Out-Of-Sample (Mark-to-Market)\nCAGR: {cagr:.2%} | MDD: {mdd:.2%} | SR: {sr:.2f}')
    plt.ylabel('Cumulative Equity')
    plt.legend()
    plt.grid(True, alpha=0.3)
    
    plot_path = r"C:\Users\Shivam Patel\.gemini\antigravity\brain\87028df4-1ec4-40e2-a989-dbc79c4b85bb\v20_equity_curve.png"
