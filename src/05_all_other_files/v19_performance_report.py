...
def generate_report():
    print("Generating V19 Mark-to-Market Performance Report...")
    trades_file = "portfolio_v19_executed_trades.parquet"
    if not os.path.exists(trades_file):
        print("Portfolio Executed Trades file not found. Run v19_portfolio_architect.py first.")
        return
        
    df_trades = pd.read_parquet(trades_file)
    df_latent = pd.read_parquet("SPY_daily_V19_latent.parquet") # To get exact dates
    
    # In V19, the trades are already filtered by the Meta-Labeler.
    ...
    plt.plot(strat_equity.index, strat_equity.values, label='V19 Strategy (OOS)', color='blue')
    plt.plot(benchmark.index, benchmark['Equity'].values, label='SPY Benchmark', color='gray', alpha=0.7)
    plt.title(f'V19 Strategy vs SPY Out-Of-Sample (Mark-to-Market)\nCAGR: {cagr:.2%} | MDD: {mdd:.2%} | SR: {sr:.2f}')
    plt.ylabel('Cumulative Equity')
    plt.legend()
    plt.grid(True, alpha=0.3)
    
    plot_path = r"C:\Users\Shivam Patel\.gemini\antigravity\brain\87028df4-1ec4-40e2-a989-dbc79c4b85bb\v19_equity_curve.png"
