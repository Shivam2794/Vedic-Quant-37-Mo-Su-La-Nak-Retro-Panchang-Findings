...
def clean_patcher():
    repo_dirs = [
        r"C:\Users\Shivam Patel\.gemini\antigravity\scratch\Vedic-Quant-37-Mo-Su-La-Nak-Retro-Panchang-Findings",
        r"C:\Users\Shivam Patel\.gemini\antigravity\scratch\EternalQuant_Github_Export"
    ]
    ...
    for rdir in repo_dirs:
        for root, _, files in os.walk(rdir):
            if ".git" in root or "brain" in root or ".gemini" in root:
                continue
            for f in files:
                if f.endswith(".py"):
    ...
    for filepath in files_to_patch:
        ...
        def yf_replacer(match):
            m = match.group(0)
            # Safe injection: only if not already there
            if "auto_adjust=" in m:
                # But wait, preserve v13 and harness
        ...
        # Eliminate duplicates if they somehow got in
        ...
        # Handle indentations if they vary
        content = re.sub(
        ...
        # Exclude known files where VAR is equity
        if "v9_ml_century_backtest_expanding" not in filepath and "v11_ml_century_backtest_expanding" not in filepath and "grid_search.py" not in filepath and "ml_parameter_optimizer.py" not in filepath and "brutal_ml_optimizer.py" not in filepath:
    ...
    print(f"CLEAN PATCHER COMPLETE: Successfully patched {patched_count} files.")
